...
SELECT @cols = @cols + '
    SUM(CASE WHEN tea.WeekStart = ''' + CONVERT(varchar(10), WeekStart, 120) + '''
             THEN tea.Logged_Hours ELSE 0 END) AS [Logged_Hours_' + CONVERT(varchar(10), WeekStart, 120) + '],

    SUM(CASE WHEN tea.WeekStart = ''' + CONVERT(varchar(10), WeekStart, 120) + '''
             THEN tea.Billable_Hours ELSE 0 END) AS [Billable_Hours_' + CONVERT(varchar(10), WeekStart, 120) + '],

    SUM(CASE WHEN tea.WeekStart = ''' + CONVERT(varchar(10), WeekStart, 120) + '''
             THEN tea.Billable_Days ELSE 0 END) AS [Billable_Days_' + CONVERT(varchar(10), WeekStart, 120) + '],

    SUM(CASE WHEN tea.WeekStart = ''' + CONVERT(varchar(10), WeekStart, 120) + '''
             THEN tea.Consumed_Days ELSE 0 END) AS [Consumed_Days_' + CONVERT(varchar(10), WeekStart, 120) + '],'
FROM Weeks
ORDER BY WeekNo;
...
TimeEntryAgg AS (
    ...
    WHERE
        ...
        AND tp.KimbleOne__EndDate__c >= DATEADD(
            WEEK,
            -26,
            ''' + CONVERT(varchar(10), @ThisWeekStart, 120) + '''
        )

        AND tp.KimbleOne__EndDate__c < '''
            + CONVERT(varchar(10), @ThisWeekStart, 120) + '''
